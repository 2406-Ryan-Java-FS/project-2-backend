...
sftp -i ~/project2.pem ec2-user@$AWS_EC2_IP << 'END'
  put pom.xml
  put -r src
END
...
ssh -i ~/project2.pem ec2-user@$AWS_EC2_IP << 'END'
...
ssh -i ~/project2.pem ec2-user@$AWS_EC2_IP << 'END'
  ...
  sudo java -jar target/RevLearn-0.0.1-SNAPSHOT.jar --spring.profiles.active=h2 --server.port=80
END
